...
enum ClientCommand {
    ...
    /// Send a best-effort DISCONNECT to the server.
    Disconnect {
...
/// A cloneable handle for sending through a **running** [`Client`].
///
/// `Client::run` consumes the client, so there is otherwise no way to send once
/// the loop is running. Apps used to work around this by cloning the transport's
/// raw `outgoing_tx` and pushing envelopes directly — which **bypasses the
/// `Client`** and therefore the reliability layer (ACK / retransmit / ordering)
/// as well as sequencing/codec/compression/encryption.
///
/// Obtain a handle with [`Client::handle`] *before* calling `run`, then send via
/// the handle so traffic is routed through the `Client` and reliability engages.
/// Sends are non-blocking (suitable for sync game-loop / UI callbacks).
#[derive(Clone)]
...
impl ClientHandle {
    ...
    /// client loop has stopped or its command queue is full.
    pub fn send_encoded(
        &self,
        route_id: u16,
        schema_hash: u64,
        ...
        self.cmd_tx
            .clone()
            .try_send(ClientCommand::SendGame {
                route_id,
                schema_hash,
                payload,
                mode,
    ...
    /// Queues a typed game message, serializing it with `codec` (use the same
    /// codec the client was configured with). See [`send_encoded`](Self::send_encoded).
    ...
    /// Queues a best-effort DISCONNECT to the server.
    pub fn disconnect(
        &self,
        reason: DisconnectReason,
        ...
        self.cmd_tx
            .clone()
            .try_send(ClientCommand::Disconnect {
                reason,
    ...
    E: Encryptor,
{
    ...
    /// Monotonic message-id counter for outgoing messages. Used directly on the
    ...
    /// Sender kept so [`Client::handle`] can mint cloneable [`ClientHandle`]s.
    cmd_tx: mpsc::Sender<ClientCommand>,

    /// Receiver drained by [`Client::run`]: app sends arriving via a `ClientHandle`.
    ...
    E: Encryptor,
{
    ...
    /// Returns a cloneable [`ClientHandle`] for sending through this client while
    /// it runs. Obtain it **before** calling [`run`](Self::run) (which consumes
    /// the client); sends made through the handle are routed via the `Client`, so
    /// the reliability layer engages (unlike pushing onto the raw transport
    /// channel).
    ...
    pub async fn run(mut self) {
        ...
        loop {
            futures::select! {
                ...
                // App sends arriving via a ClientHandle (routed through the client
                // so codec/compression/encryption are applied consistently).
    ...
    ///
    /// Dedup / ordering / ACK live in the link layer (`ReliableLink`) now, so the
    /// loop just dispatches. `MESSAGE_DROPPED` is injected by that layer.
    async fn handle_control_message(&mut self, envelope: Envelope) -> Result<(), ClientError> {
        if envelope.route_id == routes::MESSAGE_DROPPED {
            return self.handle_message_dropped(envelope).await;
        }
    ...
    /// Handles a `MESSAGE_DROPPED` notification injected by the reliability link
    /// (a reliable message exhausted its TTL / retry budget). Forwards it to the
    /// app's `dropped_tx`, if set.
    async fn handle_message_dropped(&mut self, envelope: Envelope) -> Result<(), ClientError> {
        let dropped: MessageDropped = self
            .control_codec
            .decode(&envelope.payload)
            .map_err(|e| ClientError::InvalidMessage(format!("Failed to parse dropped: {}", e)))?;
        if let Some(tx) = &mut self.dropped_tx {
            let _ = tx
                .send(ExpiredMessage {
                    seq: dropped.seq,
                    route_id: dropped.route_id,
                    correlation_id: 0,
                })
                .await;
        }
    ...
    /// Handles game messages (route_id >= 100). Dedup/reorder happen in the link
    /// layer; the loop just forwards delivered messages to the app.
    ...
    pub async fn send_message_with<T: mokosh_protocol::GameMessage>(
        ...
        // Serialize message using game codec, then hand off to the shared,
        // non-generic send path (used by both the typed API and `ClientHandle`).
        ...
        self.send_encoded(T::ROUTE_ID, T::SCHEMA_HASH, payload_bytes, mode, ttl)
            .await
    }

    /// Sends an already-codec-encoded game payload: applies compression /
    /// encryption / mode flags, assigns a monotonic `msg_id`, and forwards to the
    /// transport.
    ///
    /// This is the single outgoing-game send path (`ClientHandle` drives it via
    /// the command channel). The reliability sequence number and tracking are
    /// applied by the `ReliableLink` transport decorator (if present); on an
    /// unwrapped WebSocket transport the `msg_id` here is what the server's replay
    /// protection sees. `ttl` is carried on the wire only as the mode flags — the
    /// link layer applies a per-mode default TTL.
    async fn send_encoded(
        ...
        tracing::debug!(
            route_id,
            schema_hash = format!("{:#018x}", schema_hash),
            msg_id,
            mode = ?mode,
    ...
    /// Sends a control message (HELLO / AUTH). Over a reliable link it is sent
    /// `ReliableOrdered` (the `ReliableLink` decorator assigns the control
    /// sequence number, tracks + ACKs it); otherwise a bare `RELIABLE` bit with
    /// `msg_id` 0 (the transport itself — TCP — guarantees delivery).
    async fn send_reliable_control_message<T: serde::Serialize>(
        ...
        let flags = if self.config.reliability.is_some() {
        ...
        let envelope = Envelope::new_simple(
            ...
            0,
            route_id,
            0, // control seq assigned by the ReliableLink decorator when present
            flags,
            payload_bytes,
        );
